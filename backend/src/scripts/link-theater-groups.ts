...
// 연결은 호출하는 쪽에서 맡긴다 (운영 DB에 적용할 때 다른 연결 정보를 쓸 수 있도록 분리)
export async function migrateTheaterGroupLinks() {
...
// 이 파일을 직접 실행했을 때만 기본 연결(.env)로 마이그레이션을 수행한다
if (process.argv[1]?.includes('link-theater-groups')) {
  await connectDB()
  await migrateTheaterGroupLinks()
  process.exit(0)
}
