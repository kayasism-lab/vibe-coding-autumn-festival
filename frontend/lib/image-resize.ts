 ...
 * 긴 변 기준 최대 크기.
 *
 * 2560px은 고해상도 노트북의 가로 해상도와 같은 값이다.
 * 크게 보기(가로 1024px 자리)를 픽셀 밀도가 두 배인 화면에서 봐도 또렷하고,
 * 나중에 더 크게 보여주고 싶을 때를 위한 여유도 남는다.
 * 이보다 키우면 화면에서 차이는 느끼기 어려운데 용량만 늘어 줄이는 뜻이 흐려진다.
 */
const MAX_EDGE = 2560

/** JPEG 품질. 0.88은 눈으로 차이를 느끼기 어려우면서 용량은 크게 줄어드는 지점 */
const QUALITY = 0.88
