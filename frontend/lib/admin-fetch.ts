 ...
 * 관리 화면에서 쓰는 fetch.
 *
 * 관리 화면 로그인은 마지막 조작에서 10분이 지나면 끊긴다. 사진을 여러 장 올리는 것처럼
 * 시간이 걸리는 일을 하면 저장을 누르는 시점에 토큰이 만료돼 있을 수 있는데, 그때 그냥
 * 실패시키면 애써 올린 것이 사라진다. 그래서 한 번은 세션을 이어보고 같은 요청을 다시 보낸다.
 * 10분 넘게 쉬었다면 서버가 연장을 거절하므로, 이 재시도로도 되살아나지 않는다.
 */
...
export async function getErrorMessage(res: Response): Promise<string> {
  if (res.status === 401) {
    return '오랫동안 사용하지 않아 로그인이 풀렸습니다. 새로고침 후 다시 로그인해주세요.'
  }
